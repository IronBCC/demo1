...
def extract_rect(det, width, height, idx = 0, tresh = 0.6, classes = CLASSES):
    if len(det) < idx + 1 or det[idx, 1] < tresh:
        return None, None, None
    class_id = int(det[idx, 0])
    if not class_id and class_id >= len(classes):
        return None, None, None
...
if __name__ == '__main__':
    ...
    while (True):
        ...
        # img_origin_size = cv2.imread("image.png")
        ...
        last_cl = None
        for det in dets:
            pt1, pt2, cl = extract_rect(dets, img_origin_size.shape[1], img_origin_size.shape[0])
            if cl is None or \
            ...
            pose = estimate_pose(image, model_def, model_bin, [.8])
            ...
            if args.visualize:
                ...
                img_origin_size = cv2.rectangle(img_origin_size, pt1, pt2, (255, 0, 0))
